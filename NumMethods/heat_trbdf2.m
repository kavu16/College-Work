...
% set up matrices:
% Ustar matrix:
rstar = (1/4) * kappa* k/(h^2);
e = ones(m,1);
A = spdiags([e -2*e e], [-1 0 1], m, m);
A1star = eye(m) - rstar * A;
A2star = eye(m) + rstar * A;
...
for n = 1:nsteps
     tnp = tn + k;   % = t_{n+1}
     thalf = tn + k/2;  % = t_{n+1/2}

     % boundary values u(0,t) and u(1,t) at times tn, thalf, and tnp:

     g0n = u(1);
     g1n = u(m+2);
     g0np = utrue(ax,tnp);
     g1np = utrue(bx,tnp);
     g0half = utrue(ax,thalf);
     g1half = utrue(bx,thalf);
     ...
     rhsstar(1) = rhsstar(1) + rstar*(g0n + g0half);
     rhsstar(m) = rhsstar(m) + rstar*(g1n + g1half);

     % solve linear system:
     ustar = A1star\rhsstar;
