...
rhs(:,1) = rhs(:,1) - usoln(Iint,1)/hx^2;
rhs(:,my) = rhs(:,my) - usoln(Iint,my+2)/hx^2;
rhs(1,:) = rhs(1,:) - usoln(1,Jint)/hy^2;
rhs(mx,:) = rhs(mx,:) - usoln(mx+2,Jint)/hy^2;
...
ex = ones(mx,1);
ey = ones(my,1);
T = spdiags([(1/hy^2)*ex ((-2/hx^2)+(-2/hy^2))*ex (1/hy^2)*ex],[-1 0 1],mx,mx);
S = spdiags([ey ey],[-1 1],my,my)/hx^2;
